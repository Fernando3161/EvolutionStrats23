...
def crossover(Parents, rho):
    ...
    child = Organism(fit=fitn(child_x), x=child_x,
                     born=generation, sigma=child_sigma)
    return child
...
if __name__ == '__main__':
    ...
    for fitn in [f.rosenbruck]:
        ...
        # [m.gaus_muta, m.self_adap, m.dr_self_adap, m.evol_path, m.cma]
        for muta in [m.gaus_muta, m.self_adap, m.dr_self_adap, m.evol_path, m.cma]:
            #[ m.evol_path ]:#
            ...
            # correlation matrix which specifies correlations between dimensions
            C = np.identity(n)
            kappa = 20
            ...
            while generation < max_generation:
                ...
                children = create_children(
                    old_parent, sigma, fitn, generation, n, C, muta, lambd=lambd)
                ...
                best_parent = sorted(
                    new_parents, key=lambda x: x.fit, reverse=False)[0]
                ...
                if muta == m.self_adap or muta == m.dr_self_adap:
                    ...
                    sigma = sigma * \
                        np.exp(
                            (c_sigma / d) * (np.linalg.norm(s_sigma) / np.sqrt(n) - 1))  # eq. 10
            ...
            if plot_generation == 1:
                ...
                if best_parent.fit < 0.000000000000001:
                    plt.savefig(
                        f'Plots\{fitn.__name__}_{muta.__name__}_convert.png')
                else:
