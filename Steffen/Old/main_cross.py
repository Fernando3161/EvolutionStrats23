...
if __name__ == '__main__':
    ...
    for fitn in [f.sphere, f.rastrigen, f.rosenbruck, f.doublesum]: #[f.sphere, f.rastrigen, f.rosenbruck, f.doublesum]:
        ...
        for muta in [m.self_adap]:  # [m.gaus_muta, m.self_adap, m.dr_self_adap, m.evol_path]

            "Iteration over selection functions"
            for selec in [s.comma, s.comma_1]: #[s.plus, s.comma, s.comma_1]:
                ...
                if dt.min < timedelta(seconds=60):
                    if selec == s.plus:
                        if muta == m.gaus_muta:
                            ...
                            print(f'(1+λ) Evolution path: {old_parent.fit} from generation: {best_parent.born} Duration: {dt.seconds} seconds')
                    elif selec == s.comma:
                        if muta == m.gaus_muta:
                            print(f'(1,λ) Gaussian mutation: {old_parent.fit} from generation: {old_parent.born} Duration: {dt.seconds} seconds')
                        elif muta == m.self_adap:
                        ...
                        if muta == m.gaus_muta:
                            ...
                            print(
                                f'(1,1) Evolution path: {best_parent.fit} from generation: {best_parent.born} Duration: {dt.seconds} seconds')
                else:
